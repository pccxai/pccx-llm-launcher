...
| [`pccx_launcher/streaming_token_renderer.py`](./pccx_launcher/streaming_token_renderer.py) | Stdout token renderer for local `ResultStream` events, with optional ANSI color and per-token, end-only, or disabled flushing. Used by the guarded dry-run chat output path. |
