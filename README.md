...
For a fresh contributor checkout, use the clean-room editable install and
mock-only smoke path in [docs/INSTALL.md](./docs/INSTALL.md).
...
| [`scripts/tests/mock-e2e.sh`](./scripts/tests/mock-e2e.sh) | Clean-room mock smoke path: host probes, install preview, status summaries, dry-run launch, and dry-run chat. Requires no hardware or model assets. |
...
| [`scripts/chat-context-policy-stub.sh`](./scripts/chat-context-policy-stub.sh) | Data-only chat context-policy JSON for the Gemma 3N E4B + KV260 target. Reports disabled context-window, tokenization, token-counting, transcript, summary, truncation, context assembly, and runtime handoff gates without reading prompts, transcripts, summaries, tokenizers, model paths, runtime logs, or artifacts. |
...
| [`scripts/chat-surface-preview.sh`](./scripts/chat-surface-preview.sh) | Read-only preview of the standalone chat surface. Default output is terminal text; `--html` renders a pccx-UI branded visual shell to stdout. Both modes keep the checked chat/session contract blocked without accepting prompts, executing a model, or writing artifacts. |
...
bash scripts/chat-surface-preview.sh --model gemma3n-e4b --target kv260 --html
...
The chat empty-state fixture defines display-only placeholder slots and
disabled hints for the planned chat surface. It records local empty-state
metadata without reading prompts, responses, transcripts, session stores,
model paths, runtime logs, artifacts, or dispatching commands.
