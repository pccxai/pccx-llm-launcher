...
require_contains "$OUTPUT" "lifecycle  : blocked"
require_contains "$OUTPUT" "active     : inactive"
...
HEAD "2: controls and lifecycle operations stay disabled or non-executing"
...
require_contains "$OUTPUT" "operations : create_session=blocked"
require_contains "$OUTPUT" "restore_session=unavailable"
require_contains "$OUTPUT" "clear_session=inactive"
require_contains "$OUTPUT" "close_session=inactive"
require_contains "$OUTPUT" "export_summary=blocked"
PASS "chat/session controls and lifecycle operations are summarized"
...
require_contains "$OUTPUT" "lifecycle flags : readOnly=true"
...
require_contains "$OUTPUT" "sessionRestoreImplemented=false"
require_contains "$OUTPUT" "sessionClearImplemented=false"
require_contains "$OUTPUT" "summaryExportImplemented=false"
