...
"""Fixture tests for the chat session lifecycle contract."""
...
MODULE_PATH = ROOT / "contracts" / "chat_session_lifecycle_contract.py"
FIXTURE_PATH = (
    ROOT
    / "contracts"
    / "fixtures"
    / "chat-session-lifecycle.gemma3n-e4b-kv260-placeholder.json"
)
SCRIPT_PATH = ROOT / "scripts" / "chat-session-lifecycle-stub.sh"
...
def load_module():
    spec = importlib.util.spec_from_file_location(
        "chat_session_lifecycle_contract",
        MODULE_PATH,
    )
...
def assert_no_lifecycle_invocation_flags(source: str) -> None:
    forbidden_patterns = [
...
def test_lifecycle_matches_fixture_and_is_deterministic() -> None:
    module = load_module()
    generated = module.create_gemma3n_e4b_kv260_chat_session_lifecycle()
    fixture = json.loads(read_text(FIXTURE_PATH))

    assert generated == fixture
    assert module.chat_session_lifecycle_json(generated) == module.chat_session_lifecycle_json(generated)
    assert module.chat_session_lifecycle_json(generated).endswith("\n")
    assert json.loads(module.chat_session_lifecycle_json(generated)) == fixture
...
def test_required_fields_and_allowed_states() -> None:
    module = load_module()
    lifecycle = module.create_gemma3n_e4b_kv260_chat_session_lifecycle()
    allowed = set(module.LIFECYCLE_STATE_VALUES)

    assert tuple(lifecycle.keys()) == module.LIFECYCLE_FIELDS
    assert lifecycle["schemaVersion"] == "pccx.chatSessionLifecycle.v0"

    states = list(iter_state_values(lifecycle))
    assert states
    for state in states:
        assert state in allowed, state

    assert tuple(lifecycle["sessionIdentityPolicy"].keys()) == module.SESSION_IDENTITY_POLICY_FIELDS
    assert tuple(lifecycle["sessionStoragePolicy"].keys()) == module.SESSION_STORAGE_POLICY_FIELDS
    for operation in lifecycle["lifecycleOperations"]:
        assert tuple(operation.keys()) == module.OPERATION_FIELDS
    for transition in lifecycle["stateTransitions"]:
        assert tuple(transition.keys()) == module.TRANSITION_FIELDS
    for reason in lifecycle["blockedReasons"]:
        assert tuple(reason.keys()) == module.BLOCKED_REASON_FIELDS
    for ref in lifecycle["handoffRefs"]:
        assert tuple(ref.keys()) == module.HANDOFF_REF_FIELDS


def test_lifecycle_covers_issue_9_session_management_boundary() -> None:
    lifecycle = load_module().create_gemma3n_e4b_kv260_chat_session_lifecycle()
    operations = {
    ...
    reasons = {reason["reasonId"]: reason for reason in lifecycle["blockedReasons"]}
    refs = {ref["refId"]: ref for ref in lifecycle["handoffRefs"]}

    assert lifecycle["lifecycleState"] == "blocked"
    assert lifecycle["activeSessionState"] == "inactive"
    assert lifecycle["storageState"] == "not_configured"
    assert lifecycle["restoreState"] == "unavailable"
    assert lifecycle["exportState"] == "disabled"
    ...
    assert all(operation["enabled"] is False for operation in operations.values())
    assert operations["create_session"]["state"] == "blocked"
    assert operations["restore_session"]["state"] == "unavailable"
    assert operations["clear_session"]["sideEffectPolicy"] == "no_write"
    assert operations["export_summary"]["sideEffectPolicy"] == "no_artifact_write"
    ...
    assert {
        ...
        "model_load_not_available",
        "session_store_not_configured",
        "summary_export_boundary_absent",
...
def test_safety_flags_preserve_data_only_boundary() -> None:
    ...
    for name in [
        ...
        "sessionPersistence",
        "sessionRestoreImplemented",
        "sessionClearImplemented",
        "summaryExportImplemented",
        "touchesHardware",
        "kv260Access",
        "opensSerialPort",
        "networkCalls",
...
def test_docs_and_sources_avoid_private_data_runtime_calls_or_overclaims() -> None:
    ...
    assert_no_provider_configs(lifecycle)
    assert_no_unsupported_claims(scan_text)
    assert_no_lifecycle_invocation_flags(runtime_source)
...
test_lifecycle_matches_fixture_and_is_deterministic()
test_cli_stub_outputs_deterministic_json()
test_required_fields_and_allowed_states()
test_lifecycle_covers_issue_9_session_management_boundary()
...
print("chat/session lifecycle contract tests ok")
